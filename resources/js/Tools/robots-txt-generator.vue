<template>
    ...
    <Head>
        <meta name="description"
            content="Generate robots.txt files with sitemap support, crawl delays, and advanced directives." />
    </Head>

    <div class="max-w-6xl mx-auto bg-white shadow-lg rounded-xl p-6 space-y-6">
        <h2 class="text-2xl font-bold">Robots.txt Generator</h2>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Crawl Delay</label>
                            <select v-model="crawlDelay" class="w-full px-3 py-2 border rounded-lg">
                        ...
                        <div v-if="crawlDelay === 'custom'" class="mt-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Custom Delay (seconds)</label>
                            <input v-model="customDelay" type="number" min="0" max="3600"
                                placeholder="Enter custom delay" class="w-full px-3 py-2 border rounded-lg" />
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">User Agent</label>
                            <select v-model="userAgent" class="w-full px-3 py-2 border rounded-lg">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="space-y-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Allow/Disallow Rules</label>
                            <div class="space-y-2">
                                <div v-for="(rule, index) in rules.allow" :key="index" class="flex gap-2 items-center">
                                    <input v-model="rule.path" placeholder="Path or pattern"
                                        class="flex-1 px-3 py-2 border rounded-lg" />
                                    <select v-model="rule.type" class="w-full px-3 py-2 border rounded-lg">
                        ...
                        <!-- Disallow Rules -->
                        <div class="space-y-2">
                            <div v-for="(rule, index) in rules.disallow" :key="index" class="flex gap-2 items-center">
                                <input v-model="rule.path" placeholder="Path or pattern"
                                    class="flex-1 px-3 py-2 border rounded-lg" />
                                <select v-model="rule.type" class="w-full px-3 py-2 border rounded-lg">
                                    <option value="allow">Allow</option>
                    ...
                    <!-- Host Directives -->
                    <div class="space-y-2">
                        <label class="block text-sm font-medium text-gray-700 mb-2">Host Directives</label>
                        <div v-for="(directive, index) in hostDirectives" :key="index" class="flex gap-2 items-center">
                ...
                <!-- Sitemap Directives -->
                <div class="space-y-2">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Sitemap Directives</label>
                    <div v-for="(directive, index) in sitemapDirectives" :key="index" class="flex gap-2 items-center">
        ...
        <div v-if="result" class="space-y-6">
            <div class="bg-gray-50 rounded-lg p-6">
                ...
                <div class="bg-white rounded-lg p-4">
                    <pre class="text-sm text-gray-800 overflow-x-auto font-mono"><code>{{ result }}</code></pre>
                </div>
