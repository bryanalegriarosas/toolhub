<template>
    <meta name="description" content="Decode Base64 strings online instantly. Free tool for developers.">
    
    <div class="max-w-6xl mx-auto bg-white dark:bg-gray-800 shadow-lg rounded-xl p-6 space-y-6">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Base64 Decoder - Free Online Tool</h1>
        ...
        <div v-if="history.length" class="mt-6">
            <h3 class="font-semibold mb-3 text-gray-900 dark:text-white">Decoding History</h3>
            <ul class="space-y-2">
                <li
...
const process = () => {
    ...
    try {
        ...
        error.value = `Decoding failed: ${e.message}. Please ensure your input is valid Base64 encoded text.`;
        output.value = "";
    }
};
...
// SEO Content Data
const mainDescription = "A fast and secure online Base64 decoder that converts Base64 encoded text back to its original format. Perfect for developers, data analysts, and anyone working with encoded data. No installation required - decode directly in your browser with complete privacy.";
...
const faqs = [
    ...
    {
        question: "How do I decode a JWT (JSON Web Token)?",
        answer: "JWT tokens consist of three Base64-URL encoded parts separated by dots: header.payload.signature. To decode the token: (1) Copy your complete JWT token. (2) Split it by the dot (.) characters. (3) Take the middle section (the payload) - this contains the user data and claims. (4) Paste just that middle section into our decoder (without the dots). (5) The decoded output will be JSON showing claims like user ID, email, roles, expiration time (exp), issued time (iat), etc. You can also decode the first section (header) to see the algorithm and token type. Don't try to decode the third section (signature) as text - it's binary cryptographic data. Remember: decoding a JWT reveals its contents, but doesn't verify the signature. You still need to validate JWTs server-side for security."
    },
    {
        question: "Is it safe to use this online decoder for sensitive data?",
        answer: "Yes, our decoder is completely safe for sensitive data because all decoding happens entirely in your browser using JavaScript. Your data never leaves your computer or gets sent to our servers. There are no API calls, no server-side processing, and no data logging. The tool works offline once the page loads. However, be aware of general security practices: (1) After decoding sensitive information, use the Clear button to remove it from the screen. (2) Don't decode sensitive data on public or shared computers. (3) Remember that decoding doesn't mean the data was encrypted - if the Base64 data contains passwords or keys, it was never truly protected. (4) Browser extensions or malware could potentially capture clipboard contents, so be cautious on untrusted devices. For maximum security with highly sensitive data, consider using offline command-line tools on a secure, air-gapped system."
    },
    {
        question: "What's the maximum size of data I can decode?",
        answer: "There's no hard limit imposed by our tool, but practical browser limitations apply. Modern browsers can handle Base64 strings up to several hundred megabytes, but performance degrades with very large inputs. For optimal experience: (1) Files under 10MB decode instantly. (2) Files 10-50MB may take a few seconds and could slow down the browser temporarily. (3) Files over 50MB might cause browser lag or crashes, especially on mobile devices. For very large files, we recommend: breaking the data into smaller chunks, using the Download option instead of viewing output as text (to avoid rendering huge text blocks), or using command-line tools like 'base64 -d' on Linux/Mac or PowerShell's '[System.Convert]::FromBase64String()' on Windows for better performance with multi-gigabyte files."
    },
    {
        question: "Why does Base64 increase file size by 33%?",
        answer: "Base64 increases file size because it uses only 64 ASCII characters to represent 256 possible byte values. Here's the math: (1) Each Base64 character represents 6 bits of data (2^6 = 64 possible values). (2) Original binary data uses 8 bits per byte. (3) To encode 3 bytes (24 bits) of original data, Base64 needs 4 characters (24 bits ÷ 6 bits per character = 4 characters). (4) This gives you a ratio of 4:3, or approximately 1.33× the original size (33% increase). For example: a 300KB image becomes a 400KB Base64 string. The padding (= characters) adds slightly more overhead. This size increase is the trade-off for text compatibility - Base64 allows binary data to be transmitted through text-only systems that would otherwise corrupt or reject it."
    },
    {
        question: "Can this decoder handle Base64 with line breaks or whitespace?",
        answer: "Our decoder can handle some whitespace, but for best results, remove all spaces, tabs, and newlines before decoding. Some Base64 implementations (especially MIME encoding for emails) add line breaks every 76 characters for readability. While our tool attempts to process such input, inconsistent whitespace can cause decoding errors. If you encounter errors: (1) Copy the Base64 text into a plain text editor. (2) Use Find & Replace to remove all newlines, spaces, and tabs. (3) Ensure only valid Base64 characters remain (A-Z, a-z, 0-9, +, /, =). (4) Paste the cleaned string into our decoder. Some text editors have 'Join Lines' or 'Remove Line Breaks' features that make this easier. Alternatively, many programming languages have Base64 functions that automatically ignore whitespace during decoding."
    },
    {
        question: "What are Data URIs and how do I decode them?",
        answer: "Data URIs are a way to embed files directly in HTML, CSS, or JavaScript using Base64 encoding. The format is: data:[mimetype];base64,[base64data]. For example: 'data:image/png;base64,iVBORw0KGgo...' embeds a PNG image. To decode a Data URI: (1) Copy everything AFTER 'base64,' - this is the actual encoded data. (2) Paste just that Base64 portion into our decoder (excluding the 'data:image/png;base64,' prefix). (3) Use the Download button to save the file with the correct extension (.png, .jpg, .pdf, etc.) based on the mimetype. Data URIs are commonly used for: embedding small images in CSS to reduce HTTP requests, storing icons in JSON configuration files, creating downloadable files from JavaScript, and including attachments in single-file HTML documents. The 33% size increase makes Data URIs impractical for large files, but perfect for small assets under 10KB."
    },
    {
        question: "How do I know if my data is actually Base64 encoded?",
        answer: "Valid Base64 encoded data has these characteristics: (1) Contains only characters: A-Z, a-z, 0-9, +, /, and = (for padding). URL-safe variants use - and _ instead of + and /. (2) Length is always a multiple of 4 characters (after padding). (3) Can have zero, one, or two = characters at the end (padding), but never in the middle. (4) No spaces, newlines, or special characters (unless it's MIME-encoded with intentional line breaks). (5) Looks random but has a consistent pattern. If you see: hexadecimal (0-9, A-F only), it's probably Hex encoding. Numbers with dashes/colons (like UUIDs), it's not encoded. All uppercase or lowercase with no numbers, might be Base32. Contains special characters like {}, [], or <>, it's likely JSON or XML, not encoded. Try decoding anyway - the tool will show an error if it's not valid Base64, helping you identify the actual encoding used."
    },
    {
        question: "Why do some Base64 strings end with = or == characters?",
        answer: "The = character is padding used to ensure the Base64 string length is a multiple of 4. Here's why it's needed: Base64 encodes 3 bytes of data into 4 characters. But what if your original data isn't divisible by 3? (1) If you have 1 extra byte: the encoder creates 2 Base64 characters and adds == (two equal signs). (2) If you have 2 extra bytes: the encoder creates 3 Base64 characters and adds = (one equal sign). (3) If the data is exactly divisible by 3: no padding needed. The padding ensures the decoder knows exactly how many bytes to output. Example: 'Hello' (5 bytes) → 'SGVsbG8=' has one = because 5 bytes = 1 group of 3 + 2 extra. Some systems (especially URL-safe Base64) omit padding since the decoder can figure it out from the length, but including it is more compatible and explicit. Never remove padding manually - it can cause decoding errors."
    },
    {
        question: "Can I use this tool for command-line or automated workflows?",
        answer: "While this web tool is designed for manual, interactive use, you can adapt it for some automated scenarios: (1) Bookmarklets: create a browser bookmark that auto-fills the decoder with clipboard data. (2) Browser automation: use tools like Selenium or Puppeteer to programmatically interact with the page. (3) API integration: we don't currently offer an API, but you can use JavaScript's atob() function in your own scripts for the same functionality. For true command-line automation, we recommend: (1) Linux/Mac: echo 'SGVsbG8=' | base64 -d. (2) Windows PowerShell: [System.Text.Encoding]::UTF8.GetString([System.Convert]::FromBase64String('SGVsbG8=')). (3) Python: import base64; base64.b64decode('SGVsbG8='). (4) Node.js: Buffer.from('SGVsbG8=', 'base64').toString(). These command-line methods are faster for batch processing, scripting, and CI/CD pipelines, while our web tool is ideal for quick, one-off decoding tasks during development and debugging."
    },
    {
        question: "What should I do if the decoder says my Base64 is invalid?",
        answer: "Follow these troubleshooting steps: (1) Check for invalid characters - remove any spaces, newlines, or special characters that aren't part of the Base64 alphabet. (2) Verify the length - it should be a multiple of 4 after padding. If not, you may have copied an incomplete string. (3) Check the padding - there should be 0, 1, or 2 = characters at the END only, never in the middle. (4) Confirm the source - make sure the data is actually Base64 and not a different encoding like Hex or Base32. (5) Look for corruption - if you copied from a PDF, Word doc, or email, invisible formatting characters might be included. Try pasting into a plain text editor first. (6) Test a known valid string - try decoding 'SGVsbG8gV29ybGQh' which should give 'Hello World!' to verify the tool works. (7) Check if it's URL-safe Base64 - convert - to + and _ to / if needed. If all else fails, go back to the original source and re-copy the Base64 string carefully, ensuring you get the complete value."
    }
];

const securityNote = "All Base64 decoding happens entirely in your browser using JavaScript. Your data never leaves your computer or touches our servers, ensuring complete privacy and security. However, remember that Base64 is NOT encryption - decoded data is the same as the original and was never truly protected. Always use the Clear button after working with sensitive information, especially on shared computers.";
...
<ul class="list-disc ml-6 mb-4 space-y-2">
    <li>Stream processing: decode large files in chunks rather than all at once</li>
    <li>Web Workers: offload decoding to background threads to keep the UI responsive</li>
    <li>Server-side processing: for multi-gigabyte files, use backend services with better memory management</li>
    <li>Caching: if you decode the same data repeatedly, cache the decoded result</li>
</ul>

<h4 class="text-xl font-semibold mb-3 mt-6">Base64 in Different Programming Languages</h4>
<p class="mb-4">Understanding how to decode Base64 programmatically can help you automate workflows and integrate decoding into your applications. Here are examples in popular languages:</p>

<div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg mb-4">
    <p class="font-semibold mb-2">JavaScript/Node.js:</p>
    <pre class="bg-gray-900 text-green-400 p-3 rounded text-sm overflow-x-auto">
// Browser
const decoded = atob('SGVsbG8gV29ybGQh');

// Node.js
const decoded = Buffer.from('SGVsbG8gV29ybGQh', 'base64').toString('utf-8');
    </pre>
</div>

<div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg mb-4">
    <p class="font-semibold mb-2">Python:</p>
    <pre class="bg-gray-900 text-green-400 p-3 rounded text-sm overflow-x-auto">
import base64
decoded = base64.b64decode('SGVsbG8gV29ybGQh').decode('utf-8')
    </pre>
</div>

<div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg mb-4">
    <p class="font-semibold mb-2">PHP:</p>
    <pre class="bg-gray-900 text-green-400 p-3 rounded text-sm overflow-x-auto">
$decoded = base64_decode('SGVsbG8gV29ybGQh');
    </pre>
</div>

<div class="bg-gray-100 dark:bg-gray-700 p-4 rounded-lg mb-4">
    <p class="font-semibold mb-2">Java:</p>
    <pre class="bg-gray-900 text-green-400 p-3 rounded text-sm overflow-x-auto">
import java.util.Base64;
byte[] decoded = Base64.getDecoder().decode("SGVsbG8gV29ybGQh");
String result = new String(decoded, "UTF-8");
    </pre>
</div>

<h4 class="text-xl font-semibold mb-3 mt-6">Common Integration Patterns</h4>
<p class="mb-4">Base64 decoding is often just one step in a larger workflow. Here are common patterns where decoding fits into broader processes:</p>
